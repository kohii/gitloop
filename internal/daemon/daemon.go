...
func (d *Daemon) Run(ctx context.Context) error {
	...
	for _, repo := range d.cfg.Repositories {
		...
		go func(repo config.Repository) {
			defer wg.Done()
			d.superviseRepo(ctx, repo, recorder)
		}(repo)
	}
	wg.Wait()
	return nil
...
func (d *Daemon) superviseRepo(ctx context.Context, repo config.Repository, recorder *statusRecorder) {
	logger := d.logger.With("repo", filepath.Base(repo.Path))
	backoff := d.initialBackoff

	for ctx.Err() == nil {
		err := d.runRepoGuarded(ctx, repo, logger, recorder)
		if ctx.Err() != nil {
			return
		}
		if err == nil {
...
// parent process.
func (d *Daemon) runRepoGuarded(ctx context.Context, repo config.Repository, logger *slog.Logger, recorder *statusRecorder) (err error) {
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("panic: %v", r)
		}
	}()
	return runRepoLoop(ctx, d.gitFor(repo.Path), repo, d.hostname, logger, recorder)
}
